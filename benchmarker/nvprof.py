...
flops_in_whole_number = re.compile('(\d+)\s+flop\_count\_sp\s+Floating\s+Point\s+Operations\(Single Precision\)\s+(?:(\d+)\s+){3}')
...
def get_nvprof_counters(command):
    ...
    # print(process_err)
    match_expr1 = flops_in_whole_number.findall(process_err)
    # print(match_expr1)
    output_expr1 = [int(x) * int(y) for x, y in match_expr1]
    match_expr2 = flops_in_scientific_notation.findall(process_err)
    # print(match_expr2)
